...
You will need first of all to decide what type deployment you want to use. Our Java backend allows you to build a client server application that can be deployed on your own server or on the cloud (e.g. Amazon EC2, Cloud Docker, etc.). Alternatively client server Python based backends are also available (see [this template]((https://github.com/MetaCell/geppetto-django-template)) for Django and [this](https://github.com/MetaCell/geppetto-tornado-flask-template) for Tornado/Flask). [pygeppetto](https://github.com/openworm/pygeppetto) lets you create a Geppetto Model from Python. If you work with Jupyter notebook you also can build Geppetto applications that can run locally on your own Jupyter server. If you use a different stack you will still be able to reuse the Geppetto frontend but you will have to write your own integration code.
